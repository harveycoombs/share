...
export default function Button({ children, url, classes, transparent, loading, disabled, ...rest }: Properties) {
    ...
    let classList = `px-5 py-3 rounded-full text-[0.8rem] leading-none ${appearance} duration-150 ${loading ? "" : "cursor-pointer"} text-center select-none${classes?.length && " " + classes}`;
