...
export default function Menu({ children, small, classes, error, warning, ...rest }: Properties) {
    ...
    const classList = `${small ? "p-1.75 text-xs" : "p-2.5 text-[0.8rem]"} rounded-md border ${borderColor} bg-transparent leading-none duration-150 focus:outline-hidden focus:border-blue-500 ${classes?.length && " " + classes}`;
