...
export default function Field({ type, small, classes, error, warning, ...rest }: Properties) {
    ...
    const classList = `${small ? "px-4 py-2" : "px-5 py-2.5"} rounded-full text-[0.8rem] border ${borderColor} bg-transparent leading-none duration-150 focus:outline-hidden focus:border-blue-500 ${classes?.length && " " + classes}`;
