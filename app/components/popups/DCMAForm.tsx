...
export default function DCMAForm({ onClose }: Properties) {
    ...
    async function submit() {
        ...
        const response = await fetch("/api/dmca", {
            method: "POST",
            body: JSON.stringify({ name, email, organisation, description, captchaToken })
        });
    ...
    return (
        <Popup title="New DMCA Takedown Request" onClose={onClose}>
            <div className="w-75.5">
                ...
                <Label classes="block w-full mt-2.75">Name</Label>
                <Field classes="block w-full" warning={success === false && !name.length} onInput={(e: any) => setName(e.target.value)} />

                <Label classes="block w-full mt-2.75">Organisation</Label>
                <Field classes="block w-full" warning={success === false && !organisation.length} onInput={(e: any) => setOrganisation(e.target.value)} />

                <Label classes="block w-full mt-2.75">Email Address</Label>
                <Field type="email" classes="block w-full" warning={success === false && !email.length} onInput={(e: any) => setEmail(e.target.value)} />

                <Label classes="block w-full mt-2.75">Summary</Label>
